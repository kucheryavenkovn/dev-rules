...
## Keywords
1C, BSL, Word, docx, GRACE, markdown, documentation, standards, code-style, CI/CD, Git, RabbitMQ, Docusaurus, XML, semantic-markup

## Annotation
Генератор Word-документа «Стандарты разработки» с GRACE-DOCX v3 семантической разметкой. Читает Markdown-файлы из docs/ репозитория dev-rules, конвертирует в структурированный .docx с титульной страницей, оглавлением, главами по разделам репозитория. Инъектирует 5 GRACE XML-частей и парные закладки для 76 модулей-разделов документа.
...
```
logger.info(f"[{module_name}][{func_name}][BLOCK_NAME] message", extra={
